...
function loadGrid() {
  ...
  for (let i = 1; i <= size; i++) {
    ...
    for (let j = 1; j <= size; j++) {
      ...
      newSquareDiv.style.opacity = "0";
      newSquareDiv.textContent = "0";
...
function applyColoring() {
  ...
  document.querySelectorAll(".grid-square").forEach((item, index) => {
    item.addEventListener("mouseover", () => {
      ...
      // Update text content to display opacity value as an integer from 0 to 10
      const opacityValue = Math.round(currentOpacity * 10);
      item.textContent = opacityValue;
    });
  });
}
